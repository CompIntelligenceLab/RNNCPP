...
Add recurrence to the softmax layer. 
***/
//----------------------------------------------------------------------
void testSoftmax_rec1(int nb_batch=1)
{
	printf("\n\n\n");
	...
	Layer* d1 = new RecurrentLayer(3, "softmax");

	m->add(0,     input);
	m->add(input, d1);
	// the recurrent links are added automatically but stored inside the Recurrent Layer. 
